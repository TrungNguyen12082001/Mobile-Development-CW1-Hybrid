...
function confirmTrip(e) {
  ...
  $("#page-create #frm-confirm #departure").text(
    $("#page-create #frm-register #departure").val()
  );
  $("#page-create #frm-confirm #participant").text(
    $("#page-create #frm-register #participant").val()
  );
...
function registerTrip(e) {
  ...
  let departure = $("#page-create #frm-register #departure").val();
  let participant = $("#page-create #frm-register #participant").val();

  db.transaction(function (tx) {
    let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Date, Departure, Participant) VALUES (?, ?, ?, ?, ?, ?, julianday('${date}'))`;
    // let query1 = `txt.executeSql("ALTER TABLE items ADD Departure TEXT, Participant TEXT")`;
    // let query = `DROP TABLE Trip`;
    tx.executeSql(
      query,
      [name, destination, description, risk, departure, participant],
      transactionSuccess,
      transactionError
    );
  ...
  db.transaction(function (tx) {
    ...
    function transactionSuccess(tx, result) {
      if (result.rows[0] != null) {
        ...
        $("#page-detail #detail #departure").text(result.rows[0].Departure);
        $("#page-detail #detail #participant").text(result.rows[0].Participant);
  ...
  db.transaction(function (tx) {
    ...
    function transactionSuccess(tx, result) {
      if (result.rows[0] != null) {
        ...
        $(`#page-detail #frm-update #departure`).val(result.rows[0].Departure);
        $(`#page-detail #frm-update #participant`).val(
          result.rows[0].Participant
        );
...
function updateTrip(e) {
  ...
  let departure = $("#page-detail #frm-update #departure").val();
  let participant = $("#page-detail #frm-update #participant").val();

  db.transaction(function (tx) {
    let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Risk = ?, Departure = ?, Participant = ?, Date = julianday('${date}') WHERE Id = ?`;

    tx.executeSql(
      query,
      [name, destination, description, risk, departure, participant, id],
      transactionSuccess,
      transactionError
    );
